...
do
    ...
    then
        ...
        then
            ...
            dorado basecaller  --min-qscore 7 --emit-fastq --device cuda:0 dna_r9.4.1_e8_hac@v3.3 $current_directory/pod5/$i/ > $current_directory/fastq/${i}_call.fastq
            echo "${i} folder ${pod5files} files"
            ((i++))
        else
          sleep 30 # sleep if there is no files generated
        fi
    elif test -f $current_directory/fastq/${i}_call.fastq #just in case some one close the terminal in the middle of running guppy, this line will finish the work
    then
        dorado basecaller --min_qscore 7 --emit-fastq --device cuda:0 dna_r9.4.1_e8_hac@v3.3 $current_directory/pod5/$i/ > $current_directory/fastq/$i/
        ((i++))
